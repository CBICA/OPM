...
def GetArguments():
    ...
    parser.add_argument(
        "-i", "--Input_Image_Path", required=True, help="Image to path to slice"
    )
    parser.add_argument(
        "-c",
...
def Terminate(ConfigurationFile, manager):
    ...
    StdOutLogger.info("----------------------------------------------------")
    StdOutLogger.info(
        "Image Name                : {}   ".format(Path(manager.path).name)
    )
    ...
    StdOutLogger.info(
        "Patch Size                : {}x{}".format(
            ConfigurationFile["patch_size"][X_DIMENSION],
            ConfigurationFile["patch_size"][Y_DIMENSION],
        )
    )
    StdOutLogger.info(
        "Patch Saved               : {}   ".format(ConfigurationFile["save_patches"])
    )
    StdOutLogger.info(
        "Patching Type             : {}   ".format(ConfigurationFile["read_type"])
    )
    StdOutLogger.info(
        "Patch White Color         : {}   ".format(ConfigurationFile["white_color"])
    )
    StdOutLogger.info(
        "Patch Scale               : {}   ".format(ConfigurationFile["scale"])
    )
    StdOutLogger.info(
        "Overlap Factor            : {}   ".format(ConfigurationFile["overlap_factor"])
    )
    StdOutLogger.info(
        "Config YML File Name      : {}   ".format(
            Path(InputArguments.Configuration_Yaml_File).name
        )
    )
    StdOutLogger.info(
        "Output Directory          : {}   ".format(InputArguments.Patches_Output_Path)
    )
    StdOutLogger.info("----------------------------------------------------")
...
def Initialize():
    ...
    if not Path(InputArguments.Configuration_Yaml_File).exists():
        raise IOError(
            "Could not find the config file:\n{}".format(
                InputArguments.Configuration_Yaml_File
            )
        )
    ...
    try:
        ...
        ConfigurationFile = yaml.load(
            open(InputArguments.Configuration_Yaml_File), Loader=yaml.FullLoader
        )
    except:
...
def ParseCsvPatchesFiles(FilePath, FileName):
    ...
    DataframeColumnNames = [
        FilePathColumnName,
        X_CoordinateColumnName,
        Y_CoordinateColumnName,
    ]
    ...
    CleanedXYCoordinatesDataframe = XYCoordinatesDataframe[
        [X_CoordinateColumnName, Y_CoordinateColumnName]
    ]
    ...
    StdOutLogger.info(
        "Save Cleaned CSV Patch coordinates file: {}".format(OutputFileName)
    )
    ...
    try:
        ...
        if CsvFilePath.is_file():
            StdOutLogger.info(
                "File {} already exist, deleting...".format(CsvFilePath.name)
            )
            CsvFilePath.unlink()
...
def ProcessPatches(PatchesOutputDirectory, ConfigurationFile):
    ...
    if InputArguments.Input_XY_Coordinates_Csv_File is None:
        StdOutLogger.info(
            "User did not provide xy coordinate file, create xy coordinates"
        )
        # -----------------------------------------------------------------
        try:
            StdOutLogger.info("Generate an initial mask")
            mask, scale = generate_initial_mask(
                InputArguments.Input_Image_Path, ConfigurationFile["scale"]
            )
        except:
        ...
        try:
            StdOutLogger.info(
                "Reject patch if image dimensions are not equal to {}x{}".format(
                    ConfigurationFile["patch_size"][X_DIMENSION],
                    ConfigurationFile["patch_size"][Y_DIMENSION],
                )
            )
        ...
        # -----------------------------------------------------------------
        FileName = (
            str(Path(InputArguments.Input_Image_Path).stem) + "XYPatchCoordinates.csv"
        )
        ...
        try:
            StdOutLogger.info(
                "XY Coordinate File Name: {}".format(
                    Path(InputArguments.Input_XY_Coordinates_Csv_File).name
                )
            )
            StdOutLogger.info("Save predefined patches")
            manager.save_predefined_patches(
                PatchesOutputDirectory,
                patch_coord_csv=InputArguments.Input_XY_Coordinates_Csv_File,
                config=ConfigurationFile,
            )
        except:
...
if __name__ == "__main__":
    ...
    PatchesOutputDirectory, ConfigurationFile = Initialize()
